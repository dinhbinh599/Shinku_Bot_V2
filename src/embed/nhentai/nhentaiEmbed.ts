...
import { datetimeConverter, sortArray } from "../../utils/index.ts";
...
export const NHentaiEmbed = (
  ...
  const color = nhentai.tags.find((tag: Tag) =>
    NHentaiConstants.NHENTAI_DANGEROUS_TAGS.includes(tag.name)
  )
  ...
  return (
    new EmbedBuilder()
      ...
      .setTitle(
        `[${nhentai.id}] ${
          nhentai.title.japanese ? nhentai.title.japanese : nhentai.title.pretty
        }`
      )
      .setURL(`${NHentaiConstants.NHENTAI_BASE_URL}/g/${nhentai.id}`)
      .setImage(
        NHentaiConstants.NHENTAI_COVER(
          nhentai.media_id,
          nhentai.images.pages[0].t.split(".").pop()?.charAt(0)
        )
      )
      .setAuthor({
      ...
      .setDescription(
        nhentai.title.english ? nhentai.title.english : "No English names."
      )
      ...
      .addFields(
        {
          name: "Parodies",
          value: `\`\`\`${
            parodies.length > 0 ? parodies.join("\n") : "original"
          }\`\`\``,
        },
        {
          name: "Characters",
          value: `\`\`\`${
            characters.length > 0 ? characters.join(", ") : "original"
          }\`\`\``,
        },
        {
          name: "Artists",
        ...
        {
          name: "Tags",
          value: `\`\`\`${tags.length > 0 ? tags?.join(", ") : "N/A"}\`\`\``,
        },
        {
          name: "Favorites",
          value: nhentai.num_favorites.toString(),
          inline: true,
        },
        {
          name: "Pages count",
          value: nhentai.num_pages.toString(),
          inline: true,
        },
        {
          name: "Language",
